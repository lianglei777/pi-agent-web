...
当前 HTTP JSON Adapter 不能序列化 `undefined`。以下命令底层成功时没有业务返回值，
因此当前实现可能返回 `500 INTERNAL_ERROR`，即使命令已经执行成功：
...
这是当前实现的已知问题，不应把该 `500` 当成命令一定没有生效。
...
5. 部分无返回值 Agent Command 当前可能在执行成功后返回 `500 INTERNAL_ERROR`。
