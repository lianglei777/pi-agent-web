...
- 添加项目时默认提供绝对路径输入；目录浏览作为按需使用的辅助方式。
...
- 标题：“打开项目”。
- 默认显示“项目绝对路径”输入框，并自动聚焦。
- 用户可以粘贴或输入完整路径，按 Enter 或点击“添加项目”提交。
- “浏览目录”是次级操作；打开 Dialog 时不请求目录列表，只有点击该按钮后才进入浏览视图。
- 浏览视图复用现有的平台根位置、已注册项目快捷入口、面包屑和直接子目录列表，不打开嵌套 Dialog。
- 点击目录进入该目录；点击“选择此文件夹”只把当前位置填入输入框并返回默认视图，不立即添加项目。
- 从输入视图切换到浏览视图再返回时，保留尚未提交的手动输入。
- 取消或添加成功后清空输入并恢复默认视图；同一次页面生命周期内可以保留上次浏览位置。
- 加载期间阻止重复导航；浏览错误保留当前位置，添加错误显示在路径输入附近。
- 前端不解析、不规范化，也不猜测 Windows、Linux 或 macOS 路径格式。绝对路径、存在性、目录类型、权限、符号链接和规范路径继续由服务端校验。
- 内容区域有最大高度并内部滚动；Dialog 宽度不超过 viewport，也不受 Sidebar 的 200–420px 宽度影响。

不提供文件预览、目录创建、重命名或删除功能。
...
- Dialog 默认显示并聚焦绝对路径输入框，且不会提前请求 browse API。
- 手动输入路径可通过 Enter 和“添加项目”提交；空白输入不能提交。
- 浏览选择只填充路径并返回输入视图，用户确认后才添加项目。
- 浏览往返保留未提交输入；取消和添加成功会重置输入与视图。
- 前端不按操作系统修改用户输入，服务端返回的规范路径作为最终项目路径。
...
并在可用浏览器中完成手动路径添加、目录选择添加、移除和窄 Sidebar 工作流验证。
